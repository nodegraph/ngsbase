...
Program::Program(const std::string& vertex_source, const std::string& fragment_source, const std::string& geometry_source) {
  create_name();
  ...
  _attribute_infos = new_ff AttributeInfos(*this);
  _default_uniform_infos = new_ff DefaultUniformInfos(*this);
  _frag_data_infos = new_ff FragDataInfos(*this, fragment_source);
  GLint num_blocks = get_num_uniform_blocks();
  for (GLint i=0; i<num_blocks; ++i) {
    _block_uniform_infos.push_back(new_ff BlockUniformInfos(*this,i));
  }
